...
const EmergencyMap = dynamic(() => import('@/app/components/EmergencyMap'), { 
  ssr: false,
  loading: () => (
  ...
  steps: [
    ...
    },
    {
      ...
      mapData: {
        ...
        lng: -121.9345,
        zoom: 17,
        markerTitle: "Athletic Field Emergency Shelter"
      }
    },
  ],
                        ...
                        {currentStep === idx && (
                          <div className="bg-gray-900 rounded-xl p-6 mb-4 border-2 border-red-600/50">
                            ...
                            <div className="relative aspect-video bg-gray-800 rounded-lg border border-red-600/30 overflow-hidden">
                              <EmergencyMap
                                lat={step.mapData.lat}
                                lng={step.mapData.lng}
                                zoom={step.mapData.zoom}
                                markerTitle={step.mapData.markerTitle}
                                routeCoords={EMERGENCY_DATA.steps.map(s => ({ lat: s.mapData.lat, lng: s.mapData.lng }))}
                                dangerZone={{ lat: 37.3490, lng: -121.9395, radius: 100 }}
                              />
                              
                              {/* Map Legend - Only show on active step */}
                              <div className="absolute top-4 right-4 bg-gray-900/95 border border-red-600/40 rounded-lg p-4 backdrop-blur-sm z-[1000] shadow-xl max-w-[200px]">
                                <div className="text-white font-bold text-sm mb-3 flex items-center gap-2">
                                  <MapPin className="w-4 h-4 text-nvidia-green" />
                                  <span>Map Legend</span>
                                </div>
                                <div className="space-y-2 text-xs">
                                  <div className="flex items-center gap-2">
                                    <div className="w-3 h-3 bg-nvidia-green rounded-full animate-pulse border-2 border-white"></div>
                                    <span className="text-gray-300">Your Location</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <div className="w-3 h-3 bg-red-600 rounded-full border-2 border-white"></div>
                                    <span className="text-gray-300">Primary Fire</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <div className="text-sm">🔥</div>
                                    <span className="text-gray-300">Fire Buildings</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <div className="w-3 h-3 bg-gray-600 rounded-full border-2 border-white opacity-60"></div>
                                    <span className="text-gray-300">Smoke Haze</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <div className="w-8 h-0.5 bg-nvidia-green" style={{ borderTop: '2px dashed #76B900' }}></div>
                                    <span className="text-gray-300">Safe Route</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 bg-nvidia-green rounded-full border-2 border-white flex items-center justify-center text-[8px]">➜</div>
                                    <span className="text-gray-300">Checkpoints</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 bg-white rounded-full border-2 border-nvidia-green flex items-center justify-center text-[10px] font-bold text-nvidia-green">1</div>
                                    <span className="text-gray-300">Waypoints</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <div className="w-3 h-3 bg-green-500 rounded-full border-2 border-white opacity-40"></div>
                              ...
                              {/* Route overlay - Only show on active step if not last step */}
                              {step.number < 5 && (
                                <div className="absolute bottom-4 left-4 bg-red-950/90 border border-red-600 rounded-lg p-3 backdrop-blur-sm z-[1000]">
